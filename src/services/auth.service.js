...
const generateAccessAndRefreshToken = async (userId) => {
	try {
		...
		throw new ApiError(StatusCodes.INTERNAL_SERVER_ERROR, "Internal server error, try again after some time!", errorCodes.INTERNAL_SERVER_ERROR, false);
	}
};
...
const registerUser = async (payload) => {
	...
	if (isUserAlreadyExists) {
		throw new ApiError(StatusCodes.CONFLICT, "Email or phone number already exists", errorCodes.USER_ALREADY_EXISTS);
	}
